...
1. **Baseline (TF-IDF + LR)**: Cho kết quả tốt nhất (84% accuracy) và nhanh, phù hợp cho bài toán multi-class classification. Đây là phương pháp hiệu quả nhất trong các mô hình đã thử nghiệm.

2. **Word2Vec Average + Dense**: Đơn giản nhưng mất thông tin về thứ tự từ trong câu, chỉ đạt 39.5% validation accuracy.

3. **LSTM với Pretrained Embeddings (Non-trainable)**: 
   - Nắm bắt được thứ tự và ngữ cảnh của từ trong câu
   - Pretrained embeddings giúp khởi tạo tốt hơn
...
4. **LSTM với Pretrained Embeddings (Trainable)**: 
   ...
   - Đạt 74.91% validation accuracy, gần bằng baseline (84%)
   - Mô hình bắt đầu học từ epoch 27 và đạt đỉnh ở epoch 137 (76.02%)
   - Cho thấy việc fine-tuning embeddings có thể cải thiện đáng kể performance so với embeddings cố định
